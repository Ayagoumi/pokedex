    ...
    PokeAPI.Pokemon.resolve(name)
      .then((pokemon) => {
        ...
        const moves = pokemon.moves
          .filter((resource) => {
            return resource.version_group_details.some(
      ...
      .then((species) => {
        const description = species.flavor_text_entries.find(
          (flavor) =>
            flavor.language.name === "en" && flavor.version.name === "emerald"
        );
      ...
      .then((evolutions) => {
        ...
        function recurse(link: IChainLink) {
          ...
          link.evolves_to = link.evolves_to.filter((link) =>
            isOG(link.species.url)
          );
